...
const ParamSlider: React.FC<ParamSliderProps> = ({ label, value, min, max, step, onChange, unit = '' }) => (
  <div>
    ...
    {/* thumb 为 18×15.4px 纯白椭圆：宽度对齐最初版本（d3349014，无自定义 thumb）在 WebKit 下
        原生 thumb 的实测 18px，高度按已确认的 14:12 椭圆比例折算 15.4px；垂直居中公式：
        margin-top = (轨道高 6px − thumb 高 15.4px) / 2 = −4.7px（WebKit/Blink 会把 thumb 顶边对齐轨道顶边）；
    ...
    <input
      ...
      className="w-full h-3 bg-transparent appearance-none cursor-pointer accent-white
        ...
        [&::-webkit-slider-thumb]:[-webkit-appearance:none] [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-[18px] [&::-webkit-slider-thumb]:h-[15.4px] [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-none [&::-webkit-slider-thumb]:-mt-[4.7px]
        [&::-moz-range-track]:h-1.5 [&::-moz-range-track]:bg-zinc-800 [&::-moz-range-track]:rounded-lg
        [&::-moz-range-thumb]:w-[18px] [&::-moz-range-thumb]:h-[15.4px] [&::-moz-range-thumb]:bg-white [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-none"
    />
  </div>
);
