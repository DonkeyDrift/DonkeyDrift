...
<body>
    ...
    <script>
        ...
        // 「菜单」标题；菜单外层 panel 与「当前工作目录」bar 只去框
        // （背景/边框/内边距清零），文字内容（menu-grid / cwd label+路径）保留。
        // 单独打开 Donkey（:8090）时全部保留。
        if (isEmbedded) {
            document.querySelectorAll('.logoLink, .ghLink, .sectionTitle').forEach(function(el) {
            ...
            ['panel', 'cwdBar'].forEach(function(sel) {
