...
class DonkeyGymEnv(object):
    ...
    def __init__(self, sim_path, host="127.0.0.1", port=9091, headless=0, env_name="donkey-generated-track-v0", sync="asynchronous", conf={}, record_location=False, record_gyroaccel=False, record_velocity=False, record_lidar=False, delay=0):
        ...
        # 看门狗：记录最近一次 env.step 成功返回的时间戳。
        # gym_donkeycar 的 observe() 在模拟器断连时可能永久自旋，
        # 导致 update() 线程卡在 step() 内部、走不到健康检查；
        # 主循环（run_threaded）侧检测帧时间戳过旧则强制 _close_env()。
        self._last_step_time = time.time()
        self.watchdog_sec = 5.0
    ...
    def run_threaded(self, steering, throttle, brake=None, reconnect=False):
        ...
        # 看门狗：update 线程可能卡在 env.step() 内部（模拟器断连后
        # gym_donkeycar observe() 永久自旋），导致健康检查永远跑不到。
        # 主循环仍在周期性调用本方法，帧时间戳过旧即强制关闭 env 触发重连。
        if (self.env is not None and self.connected
                and time.time() - self._last_step_time > self.watchdog_sec):
            print(f"[DonkeyGymEnv] 看门狗：{self.watchdog_sec:.0f}s 内未收到新帧，强制重建模拟器连接")
            self._close_env()
