...
| ➤ 加载到车 | 选定该模型并请车端运行期热加载（无需重启，加载完切全自动即可推理） |
| 📋 复制路径 | 复制模型文件绝对路径到剪贴板 |
| 🗑 删除 | 删除模型及其关联的预览图和元数据 |

点击「加载到车」后，前端把模型路径发给后端，后端经 WebSocket 通知车端**运行期热加载**——车端常驻的 `PilotHolder` 会原子替换推理模型，**无需重启车端进程**。加载期间模型选择框禁用并显示「正在加载模型…」，完成后显示「模型已热加载」，切换到 AI 转向或全自动模式即可推理。模型选择同时会持久化（车端工作目录下的 `selected_model.json`，如 `~/mycar/`）；车端离线或旧版车端不支持热加载时，回退提示「模型已记录，需重启车端后生效」。

---

## 局域网主机训练（Lan Host）

> **术语约定（以车上操作视角为准，2026-09-04 定稿）**：Trainer 页面三档训练目标中，「本机」（Local Host）指手边这台运行 Web UI 的车端电脑；「局域网主机」（Lan Host）指通过 SSH 连接的远程开发电脑；「云端」（Cloud）指云服务器。内部枚举与 API 路径沿用 `local` / `mypc` / `online`，仅显示名称按上述约定。

Trainer 页面的「局域网主机」模式把另一台通过 SSH 连接的开发电脑当作训练机：后端通过 SSH 回连到这台电脑，上传 tub 数据、执行训练、再下载回模型。首次使用前需要做一次准备，页面上的「环境检测」按钮会帮你一键诊断。
