...
| ➤ 加载到车 | 选定该模型并持久化记录，车端带模型重启后生效（选择跨重启保持） |
| 📋 复制路径 | 复制模型文件绝对路径到剪贴板 |
| 🗑 删除 | 删除模型及其关联的预览图和元数据 |

点击「加载到车」后，模型选择会被持久化（车端工作目录下的 `selected_model.json`，如 `~/mycar/`）；车端在线时后端会同时下发「带模型重启」信号令其重启加载，离线时选择已记录、车端下次启动时生效。重启完成后切换到 AI 转向或全自动模式即可测试效果。
