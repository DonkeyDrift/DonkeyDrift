...
1. 只改显示字符串：`tabMyPc: 本机 → 本地主机`、`tabLocal: 车载电脑 → 本机`，并同步 `startMyPcTraining` / `startLocalTraining` / `myPcTraining` / `myPcFirstUseHint` 等派生文案；en 侧对应调整（如 `Car Computer → This Machine`、`This Computer → Local Host`，具体措辞待定）。
2. 同步第 2、3、4 类位置的测试与文档。
3. 在 `trainer.ts` 文件头加注释说明 key 名与显示语的映射约定（改名后 `startLocalTraining` 实际指「本机」，key 名与文案相反，属可接受的内部债务）。

## 风险提示与待确认

- CHANGELOG 显示该命名已**翻转过三次**（`:1455-1456`、`:1378-1380`、`:1369-1371`，上一轮明确论证过「本机=SSH 所在机、车载电脑=跑后端的机器」）。本次反馈与上轮决策方向相反，建议改前与提出者确认目标用户视角（在车上浏览器操作 vs 在开发机操作），并把最终约定写进用户手册，避免第四轮翻转。
- en 侧具体英文措辞（`Local Host` / `Dev PC` / `This Machine` / `Onboard`）需确认。
