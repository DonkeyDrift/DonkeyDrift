...
| 阶段 | 交付物 | 预计工作量 |
|-----|--------|-----------|
| M1 骨架 | 后端 drive 路由空实现 + 前端空页面 + 路由注册 | 0.5 天 |
| M2 通信通路 | WebSocket 双向打通 + MJPEG 流代理正常显示 | 1 天 |
| M3 基础控制 | 虚拟摇杆 + 键盘控制 + 模式切换 + 指示条 | 1 天 |
| M4 数据闭环 | 录制控制 + 记录数同步 + 模型下发联动 | 0.5 天 |
| M5 参数系统 | 参数面板 + 本地/服务器双端持久化 | 0.5 天 |
| M6 高级控制 | Gamepad API + 陀螺仪输入 | 0.5 天 |
| M7 校准工具 | 移植校准页面，可视化 PWM 调节 | 1 天 |
| M8 文档 | 更新用户文档，说明从旧 Tornado 迁移的步骤 | 0.5 天 |
