...
# 底盘控制接口Serial1串口配置
# 本机（AidLux 上位机）经 CH343 USB 双串口适配器（1a86:55d2）连接 ESP32：
# /dev/ttyCH343USB0 = 适配器通道0，/dev/ttyCH343USB1 = 通道1。
# 车端主板硬件 UART 默认为 /dev/ttyS4、/dev/ttyS6；若驾驶命令无响应，
# 可运行 mycar/probe_serial.py 确认帧输出端口，必要时将下面两个端口对调。
DRIVE_TRAIN_TYPE = "ARDUINO_CONTROLLER" # 底盘驱动器类型
ARDUINO_SERIAL_PORT = "/dev/ttyCH343USB0"  # 串口端口（车端主板默认：/dev/ttyS4）
ARDUINO_BAUDRATE = 115200               # 串口通信波特率（默认：115200）
ARDUINO_TIMEOUT = 1                     # 串口读取超时时间（秒），默认：1.0
ARDUINO_WRITE_TIMEOUT = 1               # 串口写入超时时间（秒），默认：1.0

# 底盘功能接口Serial2串口配置
SERIAL2_PORT = "/dev/ttyCH343USB1"      # 串口端口（车端主板默认：/dev/ttyS6）
...
DRIVE_WEBRTC_ICE_SERVERS = []  # 示例: [{"urls": ["turn:192.168.1.10:3478?transport=udp"], "username": "donkey", "credential": "donkey-turn-secret"}]
