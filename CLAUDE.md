...
- **执行器**：`actuator.py`、`pins.py`（PCA9685、PWMSteering、PWMThrottle、ArduinoFirmata 等）
- **摄像头**：`camera.py`（PiCamera、Webcam、CSICamera、V4LCamera、MockCamera、ImageListCamera 等）
- **控制器**：`controller.py`（JoystickController、LocalWebController、WebFpv、RCReceiver）、`web_controller/`
...
- **其他**：`serial_port.py`、`network.py`、`ros.py`、`file_watcher.py`、`led_status.py`、`perfmon.py`、`text_writer.py`、`coral.py`（TPU）等
